...
internal class Program
{
    ...
    private static string MAPS_DIRECTORY;
    private static string LOG_FILE;
    private static string MISSING_MAPS;
    private static string ADDED_MAPS;
    public static string INVALID_MAPS;
    ...
    public Program(ProgramOptions programOptions)
    {
        ...
        _programOptions.WorkingDirectory ??= Directory.GetCurrentDirectory();
        _programOptions.LogsDirectory ??= _programOptions.WorkingDirectory;

        MAPS_DIRECTORY = _programOptions.MapsDirectory.Replace("/", "\\");
        LOG_FILE = $"{_programOptions.LogsDirectory}\\map_updater.log";
        MISSING_MAPS = $"{_programOptions.LogsDirectory}\\missing_maps.log";
        ADDED_MAPS = $"{_programOptions.LogsDirectory}\\added_maps.log";
        INVALID_MAPS = $"{_programOptions.LogsDirectory}\\invalid_maps.log";
    ...
    private void CreateLogsDir()
    {
        try
        {
            Directory.CreateDirectory(_programOptions.LogsDirectory);
        }
        catch (Exception)
        {
            // do nothing
    ...
    {
        _mpMapsBaseIniPath = $"{_programOptions.WorkingDirectory}\\{MP_MAPS_BASE_INI_FILE}";
        ...
        _mapsPath = $"{_programOptions.WorkingDirectory}\\{MAPS_DIRECTORY}";
        ...
        _mpMapsIniPath = $"{_programOptions.WorkingDirectory}\\{MP_MAPS_INI_FILE}";
    ...
    {
        _logger.Debug("*********************************");
        _logger.Debug($"* {_programOptions.Name} .NET");
        _logger.Debug("*********************************");
    }
}
